...
class Soliloquy_Shortcode {
	...
	public function shortcode( $atts ) {
		...
		$no_js_css = '<style type="text/css">#soliloquy-container-' . sanitize_html_class( $this->slider_data['id'] ) . '{opacity:1}#soliloquy-container-' . sanitize_html_class( $this->slider_data['id'] ) . ' li > .soliloquy-caption{display:none}#soliloquy-container-' . sanitize_html_class( $data['id'] ) . ' li:first-child > .soliloquy-caption{display:block}</style>';
		$no_js     = '<noscript>';
	...
	public function get_image_slide( $id, $item, $data, $i ) {
		...
		$enable_link = ( ( isset( $data['config']['enable_link'] ) && $data['config']['enable_link'] === 1 ) || ! isset( $data['config']['enable_link'] ) ) ? true : false;
		$enable_link = ( ( isset( $data['config']['lightbox'] ) && $data['config']['lightbox'] === 1 ) || ! isset( $data['config']['lightbox'] ) ) ? true : $enable_link;
