...
abstract class Libsyn implements Iterator, Countable, ArrayAccess  {
	...
	public function getVars() {
		$this->wp_time = time();
		$dirs = explode(DIRECTORY_SEPARATOR, dirname( __FILE__ ) . DIRECTORY_SEPARATOR);
		for( $z=0; $z<(count($dirs)-6); $z++ ) $this->wp_base_dir .= $dirs[$z] . DIRECTORY_SEPARATOR;
		for( $z=0; $z<(count($dirs)-3); $z++ ) $this->plugin_base_dir .= $dirs[$z] . DIRECTORY_SEPARATOR;
		$libsyn_dir = $dirs[(count($dirs)-4)];
		$this->plugin_path = $libsyn_dir . DIRECTORY_SEPARATOR . $libsyn_dir . '.php';
		$this->plugin_admin_dir = dirname( __FILE__ ) . DIRECTORY_SEPARATOR;
